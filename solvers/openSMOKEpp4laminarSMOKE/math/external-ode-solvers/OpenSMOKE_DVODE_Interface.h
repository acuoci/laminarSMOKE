...
#define COMPLETE_ODESOLVERINTERFACE_DVODE(classname)\
bool classname::instanceFlag = false;\
classname* classname::m_this = NULL;

#define DEFINE_ODESOLVERINTERFACE_DVODE(classname)\
private:\
	\
...
public:\
	...
	\
	static void GetAnalyticalJacobianStatic(int *n, double *x, double *y, int *ml, int *mu, double *pd, int *nrowpd, double *rpar, int *ipar)\
	{\
		m_this->GetAnalyticalJacobianCallBack(n,x,y,ml,mu,pd,nrowpd, rpar, ipar);\
	}\
	\
	...
	\
	void GetAnalyticalJacobianCallBack(int *n, double *x, double *y, int *ml, int *mu, double *pd, int *nrowpd, double *rpar, int *ipar)\
	{\
