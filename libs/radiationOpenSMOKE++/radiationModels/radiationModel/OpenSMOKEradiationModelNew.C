...
Foam::autoPtr<Foam::radiation::OpenSMOKEradiationModel>
Foam::radiation::OpenSMOKEradiationModel::New
(
...
)
{
    ...
    if (cstrIter == TConstructorTablePtr_->end())
    {
        FatalErrorIn
        (
            "OpenSMOKEradiationModel::New(const volScalarField&)"
    ...
    return autoPtr<OpenSMOKEradiationModel>(cstrIter()(T));
}
...
)
{
    ...
    if (cstrIter == dictionaryConstructorTablePtr_->end())
    {
        FatalErrorIn
        (
            "OpenSMOKEradiationModel::New(const dictionary&, const volScalarField&)"
        )   << "Unknown radiationModel type "
            << modelType << nl << nl
            << "Valid radiationModel types are:" << nl
    ...
    return autoPtr<OpenSMOKEradiationModel>(cstrIter()(dict, T));
}
